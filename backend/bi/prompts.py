...
def _data_files_text(data_files) -> str:
    """アップロードされた実データ(テーブル・カラム)を一覧化。無ければ空文字。"""
    if not data_files:
        return ""
...
def get_design_prompt_step1(report_requirement: str, data_files=None) -> str:
    data_block = _data_files_text(data_files)
    data_section = (f"\n{data_block}\n\n{_DATA_MODE_NOTE}\n" if data_block else "")
...
def get_design_prompt_step2(report_requirement: str, plan: str | None, data_files=None) -> str:
    agg = " / ".join(f"{m['key']}={m['ui']}" for m in vocab.AGG)
    data_block = _data_files_text(data_files)
    data_section = (f"\n{data_block}\n\n{_DATA_MODE_NOTE}\n" if data_block else "")
    return f"""あなたは BI レポートから逆算してデータテーブルを設計する設計アシスタントです。
