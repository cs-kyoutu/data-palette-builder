...
### データの識別値（テナント固有値）は必ず**この施策相談フェーズで直接確認する**
以下のようなテナントごとに異なる値は、**プレースホルダで逃げず**にユーザーに質問して、definitionに具体値を書き込む:
- **webコンバージョン名**: 「購入完了のCVは『購入』ですか？『注文完了』ですか？」
- **ページURLパターン**: 「カートページのURLは『/cart』を含みますか？他のパターンですか？」
- **ステータス値**: 「有効会員のステータスコードは何ですか？（例: 'active', '1', '有効'）」
- **除外フラグ値**: 「メルマガ配信許可フラグで配信OKを示す値は 0 / 1 / 'true' のどれですか？」
- **購入判定方法の設計選択**: 「購入判定は受注×受注明細（商品ID単位で正確）とwebコンバージョン（リアルタイムだが顧客単位）のどちらを使いますか？」

**禁止**: 「実テーブルを見ないと確定しないので手順書生成フェーズで聞きます」という逃げ道は使わない。
**これらは要件定義の一部なので、この施策相談フェーズで必ず直接聞いて確定させる**。手順書生成フェーズには曖昧さを持ち越さない。
