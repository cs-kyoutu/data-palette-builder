...
#### 誕生日施策の加工パターン（重要）
誕生日N日前メール等の施策では「テンプレート 年齢算出」だけでは不十分。**今年の誕生日（日付型）**を作成する必要がある：
1. **抽出**: 生年月日から末尾5文字を抽出 → 「月日」カラム（例: "03-15"）
2. **IF文**: 月日が"02-29"の場合 → "02-28"に変換（うるう年対応）
3. **追加**: 加工処理実行日カラムを追加（日付型、加工処理実行日チェックON）→ 「本日日付」
4. **抽出**: 本日日付から先頭5文字を抽出 → 「本年」カラム（例: "2026-"）
5. **連結**: 「本年」+「月日」→ 「今年の誕生日_テキスト」（例: "2026-03-15"）
6. **型変換**: テキスト → 日付型 → 「今年の誕生日」
7. **時刻演算**: 今年の誕生日 - 本日日付 → 「誕生日までの日数」
※セグメント条件で「誕生日までの日数が7以下」等で絞込み可能
