...
def select_skills(input_tables: list[dict], output_mapping: dict) -> list[str]:
    ...
    # 常にロードするスキル（procedure_formatはPhase2専用なのでPhase1では読み込まない）
...
アウトプット定義の「定義」列に書いてあればそのまま採用、書いてなければ「情報不足で作れません。定義列に〇〇を追記してください」と回答する。

**質問していいのは、ナレッジ内のT1〜T3の技術確認のみ。**
- T1: リレーション項目の番号とデータ形式
- T2: 結合キー
- T3: カート/お気に入りの取得方法

**webアクセスログがインプットに含まれない、またはアウトプットにweb行動由来のカラムがない場合は、質問せず即座に設計書JSONを出力する。**
