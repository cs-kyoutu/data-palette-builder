...
def _load_industries() -> dict:
    """業界プリセットをSupabaseから読み込み、フォールバックでJSONファイル"""
    sb = _get_supabase()
    if sb:
        try:
            res = sb.table("industries").select("*").execute()
            if res.data:
...
- ページURLパターン: 施策で使うページの判定条件を**ページ種別ごとに**確認する
  - 商品詳細ページ（例: '/products/' / '/item/' / '/detail/'）
  - カートページ（例: '/cart' / '/basket'）
  - 購入完了ページ（例: '/thanks' / '/order-complete'）
  - その他、施策で判定に使うページがあれば都度確認
- ステータス値（例: 'active' / '1'）
...
def _load_strategy_templates() -> list[dict]:
    """施策テンプレートを読み込む（Supabase → JSONファイルフォールバック）"""
    sb = _get_supabase()
    if sb:
        try:
            res = sb.table("strategy_templates").select("*").execute()
            if res.data:
                return res.data
        except Exception:
            pass
...
async def create_strategy_template(entry: dict):
    ...
    sb = _get_supabase()
    if not sb:
        raise HTTPException(500, "Supabase未設定")
    if not entry.get("id"):
        entry["id"] = str(uuid.uuid4())[:8]
    sb.table("strategy_templates").upsert(entry).execute()
...
async def update_strategy_template(tpl_id: str, update: dict):
    ...
    sb.table("strategy_templates").update(update).eq("id", tpl_id).execute()
...
async def delete_strategy_template(tpl_id: str):
    ...
    if not sb:
        raise HTTPException(500, "Supabase未設定")
    sb.table("strategy_templates").delete().eq("id", tpl_id).execute()
...
async def create_industry(entry: dict):
    ...
    if not entry.get("id"):
        entry["id"] = str(uuid.uuid4())[:8]
    sb.table("industries").upsert(entry).execute()
    return {"status": "created", "id": entry["id"]}
...
async def update_industry(ind_id: str, update: dict):
    ...
    update.pop("id", None)
    update["updated_at"] = datetime.now().isoformat()
    sb.table("industries").update(update).eq("id", ind_id).execute()
    return {"status": "updated"}
...
async def delete_industry(ind_id: str):
    ...
    if not sb:
        raise HTTPException(500, "Supabase未設定")
    sb.table("industries").delete().eq("id", ind_id).execute()
    return {"status": "deleted"}
