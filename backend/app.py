...
from .generate_engine import (
    get_system_prompt_step1, get_system_prompt_step2,
    SYSTEM_PROMPT_REGEN_SUFFIX, render_processing_steps,
)
...
_DDL_STATEMENTS = [
    ...
    # Claude 呼び出しごとのトークン使用量を追記(append-only イベントログ)。
    # 集計は /api/bi/usage で都度行う(ここには合計を持たない)。
    """
    CREATE TABLE IF NOT EXISTS bi_usage (
    ...
    """,
    "CREATE INDEX IF NOT EXISTS idx_bi_usage_created_at ON bi_usage(created_at)",
    """
...
- **配信チャネル別の宛先カラム（差し込み項目に自動で入れる）**:
  ...
  - SMS → 電話番号
  - Push通知 → FCMトークン
- **配信履歴による除外**: b→dashの「メール行動ログデータ」テーブルを使って判定する（過去N日以内に配信済みの除外はこのテーブルで実現）。処理手順は「名寄せ（顧客ID×配信日時の最新）→メインデータにLEFT JOIN→**配信日時がNULLのレコードのみ残す絞込み**」の3ステップがセット。絞込みを省略すると除外が機能しないため必須
...
def _build_from_design_doc(session_id: str, session: dict, generation_data: dict, input_tables: list) -> "ChatResponse":
    ...
    実際の手順書レンダリングは generate_engine.render_processing_steps（BIの逆算設計とも共用）に委譲。"""
    ...
    if not result["ok"]:
        return ChatResponse(
            ...
            reply=f"Phase3（手順書生成）でエラー: {result['error']}\n\n設計書JSON:\n{json.dumps(generation_data, ensure_ascii=False, indent=2)[:2000]}",
            status="asking",
        )

    session["procedure_text"] = result["procedure_text"]
    session["last_file"] = result["filepath"]
    session["last_filename"] = result["filename"]
...
async def datax_favicon():
    ...
    return FileResponse(FRONTEND_PATH / "datax_favicon.png", media_type="image/png")
