...
{{
  ...
  "sections": [
    ...
    {{
      "sheet_name": "結合・加工",
      "title": "Step2: 結合・加工手順",
      "columns": ["Step", "操作種別(b→dash名)", "UI操作パス", "操作内容・設定値", "保存ファイル名", "結果の状態", "確認ポイント", "備考"],
      "rows": [
        ["1", "横統合", "データパレット → データを確認する → 統合する → カスタマイズ → webアクセスログテーブルと顧客テーブルを選択 → 横統合", "統合方法: 共通のデータのみを統合する\\n統合キー: リレーション項目_1 = 顧客ID\\n残すカラム: PV/Click日時, リレーション項目_1, リレーション項目_10, メールアドレス(メイン), メルマガ配信許可フラグ, 姓, 名", "01_アクセスログ×顧客_横統合", "顧客マスタと一致するアクセスログのみが残る", "結合前後のレコード数、NULLレコード数を確認", "更新設定: しない"],
        ["2", "絞込み", "データパレット → データを確認する → 01_アクセスログ×顧客_横統合 → 加工する → 編集方法を選択 → 絞込み", "条件1: PV/Click日時 次の期間にある 相対期間 3月前 0日前\\nAND\\n条件2: リレーション項目_10 空文字ではない", "02_直近3ヶ月カート投入_絞込み", "直近3ヶ月のカート投入データのみが残る", "期間内レコード数、カート投入データ件数を確認", "更新設定: しない"],
        ["3", "分割", "データパレット → データを確認する → 02_直近3ヶ月カート投入_絞込み → 加工する → 編集方法を選択 → 分割", "対象カラム: リレーション項目_10\\n区切り文字: ,\\n左から\\n分割数: 10\\n新カラム名: 商品ID_1, 商品ID_2, 商品ID_3, 商品ID_4, 商品ID_5, 商品ID_6, 商品ID_7, 商品ID_8, 商品ID_9, 商品ID_10", "03_商品ID分割", "カンマ区切りの商品IDが個別カラムに分割される", "分割後の各カラムの値分布を確認", "更新設定: しない"],
        ["4", "参照", "データパレット → データを確認する → 03_商品ID分割 → 加工する → 編集方法を選択 → 参照", "キーにするカラム: リレーション項目_1, 縦持ちカラム\\nソートカラム: PV/Click日時\\nソート順: 最も新しい日時", "04_商品別最終投入日時", "顧客・商品ごとの最新投入日時が取得される", "商品別の最終投入日時の妥当性を確認", "更新設定: しない"],
        ["5", "テンプレート\\n横持ちを縦持ちに変換", "データパレット → データを確認する → 04_商品別最終投入日時 → 加工する → 編集方法を選択 → 集約", "縦持ちにするカラム: 商品ID_1, 商品ID_2, 商品ID_3, 商品ID_4, 商品ID_5, 商品ID_6, 商品ID_7, 商品ID_8, 商品ID_9, 商品ID_10\\n残すカラム: PV/Click日時, リレーション項目_10, メールアドレス(メイン), メルマガ配信許可フラグ, 姓, 名", "05_顧客別集約", "顧客ごとの最終カート投入日と基本情報が集約される", "顧客数の一致、集約後の値の妥当性を確認", "更新設定: しない"],
        ["6", "横統合", "データパレット → データを確認する → 統合する → カスタマイズ → 04_商品別最終投入日時と商品テーブルを選択 → 横統合", "統合方法: 先に選択したデータに対して統合する\\n統合キー: 縦持ちカラム = 商品ID\\n残すカラム: 全カラム", "06_商品情報結合", "商品情報が結合される（商品情報がない場合はNULL）", "商品情報の結合率、NULL件数を確認", "更新設定: しない"],
        ["7", "連結", "データパレット → データを確認する → 05_顧客別集約 → 加工する → 編集方法を選択 → 連結", "連結カラム1: 姓\\n連結カラム2: 名\\n区切り文字: スペース\\n新カラム名: 名前\\n元カラム: 残さない", "07_名前結合", "姓と名がスペース区切りで結合される", "名前の結合結果を確認", "更新設定: しない"],
        ["", "参照", "データパレット → データを確認する → 05_顧客別集約 → 加工する → 編集方法を選択 → 参照", "処理パターン: グループ内の最後の値\\nグループ化カラム: リレーション項目_1\\nソートカラム: PV/Click日時\\nソート順: 昇順\\n値取得カラム: PV/Click日時\\n新カラム名: 最終カート投入日時", "", "", "", ""],
        ["8", "時刻演算", "データパレット → データを確認する → 07_名前結合 → 加工する → 編集方法を選択 → 時刻演算", "左辺カラム: 現在日時\\n演算子: -\\n右辺カラム: 最終カート投入日\\n単位: 時間\\n新カラム名: 最終カート投入日からの経過時間", "08_経過時間計算", "最終投入日からの経過時間（時間単位）が計算される", "経過時間の値の妥当性を確認", "更新設定: しない"],
        ["9", "テンプレート\\n縦持ちを横持ちに変換", "データパレット → データを確認する → 06_商品情報結合 → 加工する → 編集方法を選択 → ランキング", "キーにするカラム: リレーション項目_10, 最終カート投入日時, 最終カート投入日からの経過時間, メールアドレス(メイン), メルマガ配信許可フラグ, 姓, 名, 名前\\n横持ちカラム: 商品ID, 商品名, 商品詳細ページURL, 商品価格, 商品画像URL\\nソート順: 降順\\nソートするカラム: PV/Click日時\\n何番目まで並べるか: 上位5位まで", "09_商品ランキング", "顧客ごとに商品の投入順位が付けられる", "ランキングの付与状況を確認", "更新設定: しない"]
      ]
    }},
    {{
      "sheet_name": "最終確認",
...
### パターン2: 同一ファイルの連続加工
Step 7: 連結（姓+名）→ 参照（最終カート投入日時）のように、同一ファイルで連続加工する場合：
- 1つ目にStep番号と保存ファイル名を記載
- 2つ目以降はStep番号を空、保存ファイル名も空にする

### パターン3: ステップ数は最小限
上記参考例のように**9ステップ程度**で完結させる。不要な中間ステップ（追加→IF文→集約の冗長展開等）は作らない。
